...
- PENDING: Phase 4 timers add an injectable clock, `timer` request mode, and `deadline_at` request metadata; expiration is driven by runtime advancement rather than kernel side effects.
- PENDING: Deadline expiration marks pending requests `expired` and emits deterministic `request.expired` events with request correlation metadata for frontend timeout displays.
